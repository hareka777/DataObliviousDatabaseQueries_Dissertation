...
def experiment(database, query_list, oblivious_memory_size, block_size):
    oblidb = ObliDB_queries.ObliDB(database, oblivious_memory_size)
    for query in query_list:
        print(query)
        query = query.strip('\n')
        oblidb.sort_merge_join(query=query)
        cost_tracker.ObliviousTracker.query_completed()

    cost_tracker.ObliviousTracker.log_experiment_results('Results\ObliDB\Join\OM\om_join_block_' +str(block_size) + 'mem_' + str(oblivious_memory_size) + '.txt ')
